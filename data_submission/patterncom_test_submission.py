...
from data.patterncom_dataset import PatternComDataset
...
# Calculate mAP by test attribute using the improved function
def calculate_map_by_attribute_improved(data, predictions, attribute, at=[1, 5, 10, 15, 20]):
    """
    Calculate mean Average Precision (mAP) by test attribute using the improved function
    """
    # Filter data by attribute
    indices = [i for i, item in enumerate(data) if item['attribute'] == attribute]
    gt_list = [data[i]['label'] for i in indices]
    pred_list = [predictions[i] for i in indices]
...
def generate_patterncom_test_submissions(file_name, model, preprocess, device):
    type = 'test'
    classic_test_dataset = PatternComDataset(type, 'classic', preprocess)
    index_features, index_names, _ = extract_index_features(classic_test_dataset, model, return_local=False)
    relative_test_dataset = PatternComDataset(type, 'relative', preprocess)
    pairid_to_predictions = generate_patterncom_test_dicts(relative_test_dataset, index_features, index_names, model, device)

    # print(f"Saving PatternCom test predictions")
    # if type == 'val':
    #     name = 'val_' + file_name
    # else:
    #     name = file_name
    # output_file = f"./submission/patterncom_final/{name}.json"
    # with open(output_file, 'w') as file:
    #     json.dump(pairid_to_predictions, file)
    # print('Results are written to file ', output_file)
    data =  json.load(open('/home/hle/CIR/data/PatternNet/test.json', 'r')) 
    ground_truths = [i['label'] for i in data]
    predictions = list(pairid_to_predictions.values())

    attributes = set([i['attribute'] for i in data])   
    ...
    for attribute in attributes:
        scores = calculate_map_by_attribute_improved(data, predictions, attribute)
        for metric, value in scores.items():
            if metric == 'AP':
                # print(f"{attribute} {metric}: {value}")
                sum += value
    print(f"Average AP: {sum / len(attributes)}")
 
def generate_patterncom_test_dicts(relative_test_dataset, index_features, index_names, model, device):
    ...
    print(f"Compute PatternCom prediction dicts")
    ...
    for i in range(len(pairs_id)):
        ...
        topk = remove_duplicate(sorted_results)[:200]
        # try: 
...
def generate_patterncom_test_predictions(relative_test_dataset: PatternComDataset, model, device) -> Tuple[torch.tensor, List[str], List[List[str]], List[str]]:
...
def main_patterncom(cfg):
    ...
    generate_patterncom_test_submissions(cfg.submission_name, model, preprocess, device=device)
